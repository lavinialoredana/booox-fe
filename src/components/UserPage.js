    ...
    <Fragment>
      <div className="user-page">
        ...
        <div className="user-page-1">
          <div className="user-page-1-child">
            <Link to="/userprofile">Profile Area</Link>
          </div>
          <div
            className="user-page-1-child user-page-1-child-2 "
            onClick={showBooksFunction}
          >
            See all my books <b>↓</b>
          </div>
          <div className="user-page-1-child">
            <Link to="/bookupload">Upload My Book</Link>
          </div>
          <div className="user-page-1-child" onClick={showMyRequest}>
            Request I made
          </div>
          <div className="user-page-1-child">
        ...
        <div className="user-page-2">
          <div className="user-page-2-details">
            <div className="user-page-1-child">
              Total Books owned : {bookDetails.length}
            </div>
            <div>
              <div>
                <h2 className="user-page-1-child-all-requests">
                  Lists Of Request I got{" "}
                </h2>

                {myPendingRequests.map((any) => {
                  return any.status === "Pending" ? (
                    <div className="user-page-1-child-pendingrequests requestStatus">
                      <div>
                        <b>Request From User :</b> {any.name}
                      </div>
                      <div>
                        <b>Request On Book :</b>
                        {any.title}
                      ...
                      <div>
                        <b>Request Status : </b> {any.status}
                      </div>
                      <div>
                        <b>Requesting User Email :</b> {any.email}
                      </div>
                      <div>
                        <Link to={"/mybook/" + any.id}>
          ...
          <div className="user-page-2-books"></div>
        </div>
        <div className="single-book-details-wrap">
          {displayMyBook
            ? bookDetails.map((any) => (
                <div key={any.id} className="single-book-details">
                  ...
                  <div className="single-book-photo">
                    <img
                      style={{ width: "100%", height: "inherit" }}
                      className="lower-body-image"
                      src={image1}
                      alt=""
                    ></img>
                  </div>
          ...
          {displayMyRequests ? (
            <div className="single-book-details myrequests">
              ...
              {myOutgoingPendingRequest.map((any) => {
                ...
                return (
                  <div
                    key={any.id}
                    className={`user-page-1-child requestStatus ${color}`}
                  >
                    <div>
                      <b>Request To User :</b> {any.name}
                    </div>{" "}
                    <div>
                      <b>Request On Book Title :</b>
                      {any.title}
                    </div>
                    <div>
                      <b>Request Status : </b> {any.status}
                    ...
                    <div>
                      <b> Book Owner Email :</b> {any.email}
                    </div>
                  </div>
                );
              })}
